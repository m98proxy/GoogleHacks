...
void main()
{
	...
	vec2 q = gl_FragCoord.xy / vec2(800, 600);
	vec2 p = -1.0 + 2.0 * q;
	vec2 qq;
	vec2 r;
	float color = pattern2(p, qq, r, seed);

	vec4 c = vec4(color, color, color, 1.0); // areas that are transparent are black
											 //vec4 c = vec4(color, color, color, color); // areas that are black are transparent
